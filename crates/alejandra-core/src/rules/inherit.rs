pub(crate) fn rule(
    ...
    let children: Vec<crate::trivia::Child> =
        crate::trivia::new(build_ctx, node).collect();
    ...
    for trivia in child.trivialities {
        match trivia {
            crate::trivia::Trivia::Comment(text) => {
                steps.push_back(crate::builder::Step::Comment(text));
                steps.push_back(crate::builder::Step::NewLine);
                steps.push_back(crate::builder::Step::Pad);
            }
            crate::trivia::Trivia::Newlines => {}
        }
    }
        ...
        if vertical {
            ...
            if let Some(text) = child.inline_comment {
                ...
                if matches!(
                    ...
                    None | Some(crate::trivia::Trivia::Comment(_))
                ) {
                    steps.push_back(crate::builder::Step::Pad);
                }
                ...
                || matches!(
                    child.trivialities.front(),
                    Some(crate::trivia::Trivia::Comment(_))
                )
            {
                steps.push_back(crate::builder::Step::NewLine);
                steps.push_back(crate::builder::Step::Pad);
            ...
            while let Some(trivia) = trivia_iter.next() {
                match trivia {
                    crate::trivia::Trivia::Comment(text) => {
                        ...
                        if matches!(
                            trivia_iter.peek(),
                            Some(crate::trivia::Trivia::Newlines)
                        ) {
                            continue;
                        }
                    }
